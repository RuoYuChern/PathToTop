...
前面介绍了计算机的关键组成，那么一个程序（进程）如何跟这些关键硬件交互的？这里需要了解操作系统。操作系统充当应用程序和计算机硬件之间的接口，其设计有5个关键的目标：

1. 通用性： 定义标准接口，易于各种硬件集成。
2. 易用性：使计算机易于使用，易于程序开发、运行。
3. 实时性：能够实时响应事件，满足程序运行实时要求。
4. 可扩性：易于开发新的系统功能。
5. 安全性：权限管理、进程隔离。
...
1. 程序开发：提供应用程序开发工具、系统API库（服务）帮助程序员实现特定功能的程序。
2. 程序运行：加载指令和数据到内存、初始化I/O设备和其它资源，对用户处理进行调度。
3. I/O设备访问：隐藏I/O设备特有的指令集和控制信号，提供统一、通用的接口，使得程序开发者能够使用易用的接口进行读、写来访问这些设备。
4. 文件访问及其控制：提供保护机制来控制多租户（用户）对文件的访问。
5. 系统访问：控制对整个系统的访问以及对某个特殊系统资源的访问，即保护了资源和数据，又避免了未授权用户的访问，同时解决了资源竞争时的冲突问题。
6. 错误检测和响应：提供响应以清除错误，使各种发生的错误对正在运行的应用程序影响最小。
7. 监控和审计：可以收集各种资源的审计（统计），比如CPU、硬盘、访问记录，从而进行系统的监控。

那么操作系统如何实现以上的功能，接着我们来介绍操作系统设计层次。
